    ...
    <div className="min-h-screen overflow-x-hidden text-white selection:bg-white/20" style={{ backgroundColor: theme.bg }}>
      ...
      <div className="relative z-10 pt-24 lg:pt-32">
        ...
        <section className="mx-auto max-w-[1800px] flex flex-col lg:grid lg:grid-cols-[1.5fr_1fr] lg:gap-12 xl:gap-20">
          ...
          {/* Right: Product Info & Accordions - Mobile Second */}
          <div className="relative mt-8 lg:mt-0 px-6 lg:pr-12 xl:pr-20 flex flex-col">
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, ease: [0.16, 1, 0.3, 1] }}>
              ...
              {product.tagline && (
                <p className="mt-3 text-lg text-white/70">{product.tagline}</p>
              )}
            ...
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 0.2, ease: [0.16, 1, 0.3, 1] }}
              ...
              {/* Compact Price Box */}
              <div className="flex">
                <div className="inline-flex items-center gap-5 rounded-sm border px-5 py-3 backdrop-blur-md" 
              ...
              <div className="mt-6 flex flex-col gap-3 sm:flex-row">
                <div className="flex h-14 w-full sm:w-32 items-center justify-between rounded-sm border px-4"
                  style={{ borderColor: `${theme.accent}40`, backgroundColor: "rgba(0,0,0,0.3)" }}>
                  <button onClick={() => setQty(Math.max(1, qty - 1))} className="transition hover:scale-110 p-2" style={{ color: theme.accent }}>
                    <Minus className="h-4 w-4" strokeWidth={2} />
                  </button>
                  <AnimatePresence mode="popLayout">
                    <motion.span key={qty} initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 10, opacity: 0 }}
                      className="w-8 text-center text-base font-medium text-white">{qty}</motion.span>
                  </AnimatePresence>
                  <button onClick={() => setQty(qty + 1)} className="transition hover:scale-110 p-2" style={{ color: theme.accent }}>
                  ...
                  <AnimatePresence mode="wait">
                    {status === "idle" && (
                      <motion.span key="idle" initial={{ y: 15, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -15, opacity: 0 }}
                        className="text-[11px] font-bold uppercase tracking-[0.2em]">Add to Bag</motion.span>
                    )}
                    {status === "loading" && (
              ...
              <div className="mt-6 flex gap-8">
                ...
                <button className="group flex items-center gap-2 text-[10px] uppercase tracking-[0.2em] text-white/60 transition-colors hover:text-white">
              ...
              <AccordionItem title="Ingredients" open={activeTab === "ingredients"} onClick={() => setActiveTab(activeTab === "ingredients" ? "" : "ingredients")} accent={theme.accent}>
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  {product.ingredients.map((i: string) => (
                    <div key={i} className="rounded-sm border px-4 py-3 backdrop-blur-sm transition"
                      ...
                      onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.backgroundColor = "rgba(0,0,0,0.2)"; }}>
                ...
                <div className="mt-5 flex gap-5">
                  {["Clean", "Vegan", "Cruelty-Free"].map((tag) => (
                    <span key={tag} className="text-[9px] font-bold uppercase tracking-[0.25em]" style={{ color: theme.accent }}>{tag}</span>
                  ))}
                </div>
              </AccordionItem>

              <AccordionItem title="How to Use" open={activeTab === "benefits"} onClick={() => setActiveTab(activeTab === "benefits" ? "" : "benefits")} accent={theme.accent}>

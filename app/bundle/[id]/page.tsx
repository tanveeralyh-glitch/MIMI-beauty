  ...
  return (
    <div className="border-b border-white/10">
      <button
        ...
        onClick={onClick}
        aria-expanded={open}
        className="flex min-h-16 w-full items-center justify-between py-5 text-left font-display text-lg"
      >
        {title}
        <ChevronDown className={`size-5 text-white/50 transition-transform ${open ? "rotate-180" : ""}`} />
      </button>
...
export default function BundleDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!bundle) {
    return (
      <div className="grid min-h-screen place-items-center bg-background text-foreground">
        <p>
          Bundle not found. <Link className="underline" href="/bundles">Back to bundles</Link>
        </p>
      </div>
    );
  }
  ...
  const addBundle = () => {
    ...
    add(
      {
        ...base,
        slug: `${bundle.id}-${packaging}-${oil}`,
        name: bundle.name,
        tagline: bundle.description,
        category: "BUNDLE",
        collection: "Mimi Sets",
        price: total,
        size: packaging === "gift" ? "Bundle · Gift Packaging" : "Bundle",
        image: bundle.image,
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-background text-foreground">
      <div className="mx-auto max-w-7xl px-5 pb-24 pt-28 sm:px-8 lg:px-12 lg:pt-36">
        <Link href="/bundles" className="mb-8 inline-flex items-center gap-3 text-[10px] uppercase tracking-[.28em] text-white/50 hover:text-white">
        ...
        <main className="grid gap-10 lg:grid-cols-[minmax(0,1.1fr)_minmax(360px,.9fr)] lg:gap-20">
          <div className="contents lg:block">
            <h1 className="order-1 font-display text-[clamp(2.5rem,7vw,6rem)] leading-[.92] lg:hidden">{bundle.name}</h1>

            <div className="order-2 flex min-h-[52vh] items-center justify-center border border-gold/15 bg-black/20 p-5 sm:min-h-[620px] sm:p-12 lg:order-none">
              <img
                src={bundle.image}
                alt={bundle.name}
                className="max-h-[620px] w-full object-contain drop-shadow-[0_30px_35px_rgba(0,0,0,.6)]"
              />
            </div>
          ...
          <section className="order-4 lg:order-none lg:pt-8">
            ...
            <h1 className="mt-5 hidden font-display text-[clamp(3rem,6vw,6rem)] leading-[.92] lg:block">{bundle.name}</h1>
            ...
            <div className="mt-8 inline-flex items-baseline gap-5 border border-gold/40 bg-gold/5 px-5 py-4">
              ...
              <span className="text-[11px] uppercase tracking-[.2em] text-white/50">Bundle</span>
            </div>
            ...
            <div className="mt-8 flex flex-col gap-4">
              ...
              {containsOil && (
                <div className="flex flex-col gap-3">
                  <p className="text-[10px] uppercase tracking-[.25em] text-white/50">Choose body oil</p>
                  <div className="grid grid-cols-2 gap-2">
                    {oilNames.map((item) => (
                      <button
                        type="button"
                        key={item.id}
                        onClick={() => setOil(item.id)}
                        className={`border px-3 py-3 text-left text-sm ${
                          oil === item.id ? "border-gold text-gold" : "border-white/15 text-white/70"
                        }`}
                      >
                        {item.name}
                      </button>
            ...
            <div className="mt-7 flex flex-wrap items-center gap-4">
              ...
              <button
                type="button"
                onClick={addBundle}
                className="flex min-h-12 flex-1 items-center justify-center gap-2 bg-gold px-7 text-xs font-semibold uppercase tracking-[.2em] text-background sm:flex-none"
              >
                {added ? (
                  <>
  ...
  return (
    <div>
      {[
        ...
        ["how", "How to use", "Build your ritual from the products included in your set, then enjoy the formulas in sequence."],
        ["details", "Bundle details", "A thoughtful Mimi Beauty edit designed to work beautifully together."],
      ].map(([id, title, text]) => (
        <Accordion
          key={id}
          title={title}

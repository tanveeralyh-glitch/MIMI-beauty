    ...
    <div className="min-h-screen overflow-x-hidden text-white selection:bg-white/20" style={{ backgroundColor: theme.bg }}>
      ...
      <div className="relative z-10 pt-24 lg:pt-32">
        <div className="mx-auto max-w-[1800px] px-6 pb-6 lg:px-12 xl:px-20">
          <div className="flex items-center gap-6">
            <Link href="/shop" className="group flex items-center gap-3 text-[10px] uppercase tracking-[0.3em] text-white/50 transition-colors hover:text-white">
              <span
                className="flex h-8 w-8 items-center justify-center rounded-full border border-white/10 bg-white/5 transition-colors"
                onMouseEnter={(e) => {
                  const el = e.currentTarget as HTMLElement;
                  el.style.borderColor = theme.accent;
                  el.style.backgroundColor = theme.accentMuted;
        ...
        <section className="mx-auto max-w-[1800px] flex flex-col lg:grid lg:grid-cols-[1.5fr_1fr] lg:gap-12 xl:gap-20">
              ...
              <div className="mt-5 grid grid-cols-4 gap-3">
                {gallery.slice(0, 4).map((image, index) => (
                  <button
                    ...
                    className={`border p-2 ${active === index ? "border-white/30" : "border-white/10 bg-black/10"}`}
                  >
          ...
          <div className="relative mt-8 lg:mt-0 px-6 lg:pr-12 xl:pr-20 flex flex-col">
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, ease: [0.16, 1, 0.3, 1] }}>
              <h1 className="font-display text-[clamp(2.5rem,6vw,5rem)] leading-[1] tracking-tight text-white">{product.name}</h1>
              {product.collection && (
                <div className="mt-4 flex items-center gap-3">
                  <span className="text-[10px] sm:text-[11px] font-medium uppercase tracking-[0.4em]" style={{ color: theme.accent }}>{product.collection}</span>
                  <span className="h-[1px] w-6" style={{ backgroundColor: theme.accent }} />
                  <span className="text-[10px] sm:text-[11px] font-medium uppercase tracking-[0.4em]" style={{ color: theme.accent }}>Collection</span>
                </div>
              )}
              {product.tagline && <p className="mt-3 text-lg text-white/70">{product.tagline}</p>}

              <div className="mt-8">
                <div className="text-base leading-relaxed text-white/80 md:text-lg whitespace-pre-wrap">{product.description}</div>
              </div>
            </motion.div>
            ...
            >
              <div className="flex">
                <div
                  className="inline-flex items-center gap-5 rounded-sm border px-5 py-3 backdrop-blur-md"
                  style={{ borderColor: `${theme.accent}40`, backgroundColor: "rgba(0,0,0,0.2)" }}
                >
                  <span className="font-display text-xl sm:text-2xl tracking-tight text-white">Rs. {product.price}</span>
                  <span className="h-4 w-px bg-white/20" />
                  <span className="text-[10px] sm:text-xs uppercase tracking-[0.2em] text-white/70">{product.size}</span>
                </div>
              </div>

              <div className="mt-6 flex flex-col gap-3 sm:flex-row">
                <div
                  className="flex h-14 w-full sm:w-32 items-center justify-between rounded-sm border px-4"
                  style={{ borderColor: `${theme.accent}40`, backgroundColor: "rgba(0,0,0,0.3)" }}
                >
                  <button type="button" onClick={() => setQty(Math.max(1, qty - 1))} className="transition hover:scale-110 p-2" style={{ color: theme.accent }}>
                  ...
                  <AnimatePresence mode="popLayout">
                    <motion.span
                      ...
                      exit={{ y: 10, opacity: 0 }}
                      className="w-8 text-center text-base font-medium text-white"
                    >
                      {qty}
                    </motion.span>
                  </AnimatePresence>
                  <button type="button" onClick={() => setQty(qty + 1)} className="transition hover:scale-110 p-2" style={{ color: theme.accent }}>
                ...
                <MagneticButton
                  ...
                  className="flex h-14 w-full flex-1 items-center justify-center rounded-sm sm:w-auto transition-all duration-300 hover:brightness-110"
                  style={{
              ...
              <div className="mt-6 flex gap-8">
                <button
                  ...
                  style={{ color: wishlisted ? theme.accent : "rgba(255,255,255,0.6)" }}
                >
                  <Heart
                ...
                <button type="button" className="group flex items-center gap-2 text-[10px] uppercase tracking-[0.2em] text-white/60 transition-colors hover:text-white">
                  <Share2 className="h-4 w-4 transition-transform group-hover:scale-110" strokeWidth={1.5} /> Share
                </button>
              </div>
            </motion.div>

            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1, delay: 0.4 }} className="mt-12 pb-16">
              {product.directions && (
                <AccordionItem
                  title="Directions"
                  ...
                  accent={theme.accent}
                >
                  <p className="text-base text-white/80 leading-relaxed whitespace-pre-wrap">{product.directions}</p>
                </AccordionItem>
              )}
              ...
              >
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  {product.ingredients.map((i: string) => (
                    <div
                      key={i}
                      className="rounded-sm border px-4 py-3 backdrop-blur-sm transition"
                ...
                <div className="mt-5 flex gap-5">
                  {["Clean", "Vegan", "Cruelty-Free"].map((tag) => (
                    <span key={tag} className="text-[9px] font-bold uppercase tracking-[0.25em]" style={{ color: theme.accent }}>
                      {tag}
                    </span>
                  ))}
                </div>
                ...
                <ul className="space-y-4">
                  {product.benefits.map((b: string) => (
                    <li key={b} className="flex items-start gap-4">
                      <span className="mt-1.5 grid h-2 w-2 shrink-0 place-items-center rounded-full" style={{ backgroundColor: theme.accentMuted }}>
              ...
              {product.storageCaution && (
                <AccordionItem
                  ...
                  accent={theme.accent}
                >
                  <p className="text-base text-white/80 leading-relaxed whitespace-pre-wrap">{product.storageCaution}</p>
                </AccordionItem>
              )}
            </motion.div>
          </div>
      ...
      <section className="relative z-10 border-t py-24 md:py-32 backdrop-blur-2xl" style={{ borderColor: `${theme.accent}15`, backgroundColor: `${theme.bg}cc` }}>

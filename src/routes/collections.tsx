...
export const Route = createFileRoute("/collections")({
  ...
  component: () => (
    <>
      ...
      <section className="mx-auto grid max-w-[1400px] gap-4 px-6 pb-16 md:pb-32 md:grid-cols-2">
        {collections.map((c, i) => (
          <motion.div key={c.slug} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.06 }} className={`group relative min-h-[420px] overflow-hidden rounded-3xl p-6 bg-gradient-to-br md:p-10 ${c.tone}`}>
            ...
            <Link to="/shop" className="absolute bottom-6 left-6 inline-flex items-center gap-2 rounded-full bg-background/95 px-5 py-2.5 text-sm text-foreground md:bottom-10 md:left-10">

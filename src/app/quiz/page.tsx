...
const steps = [
  { q: "How would you describe your skin?", opts: ["Dry", "Oily", "Combination", "Sensitive"] },
  {
    ...
    opts: ["Dullness", "Fine lines", "Breakouts", "Barrier repair"],
  },
  {
      ...
      <AnimatePresence mode="wait">
        {!done ? (
          ...
          >
            ...
            <h2 className="mt-3 font-display text-2xl leading-tight sm:text-4xl md:text-5xl">
              {steps[step].q}
            </h2>
            ...
            <ul className="mx-auto mt-6 max-w-md text-left text-sm text-muted-foreground">
              {answers.map((a, i) => (
                <li key={i} className="flex justify-between border-b border-border py-2">
                  {steps[i].q.replace("?", "")} <span className="text-foreground">{a}</span>
                </li>
              ))}
            </ul>
            <Link

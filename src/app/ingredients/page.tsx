...
export default function Page() { return (
    <>
      ...
      <section className="mx-auto grid max-w-[1400px] gap-4 px-6 pb-16 md:pb-32 md:grid-cols-2 lg:grid-cols-3">
        {ingredientsShowcase.map((it, i) => (
          <motion.div key={it.name} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.06 }} className="group relative aspect-[4/3] overflow-hidden rounded-3xl border border-border bg-gradient-to-br from-secondary to-background p-5 md:p-8">
            <div className="absolute -right-8 -top-8 h-40 w-40 rounded-full bg-gold/20 blur-2xl transition group-hover:bg-gold/40" />
            <div className="absolute right-6 top-6 h-16 w-16 rounded-full border border-gold/40">
